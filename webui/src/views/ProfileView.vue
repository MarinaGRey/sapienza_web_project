...
export default {
	...
	methods: {
		...
		async banClick(){
            try{
                if (this.banStatus){
                    // Delete ban: /users/:id/banned/:banned_id
                    await this.$axios.delete("/users/"+localStorage.getItem('token')+"/banned/"+ this.$route.params.id);
                    this.loadInfo()
                }else{
                    // Put ban: /users/:id/banned/:banned_id
                    await this.$axios.put("/users/"+localStorage.getItem('token')+"/banned/"+ this.$route.params.id);
		...
		this.postCnt = response.data.photos != null ? response.data.photos.length : 0

	        this.followStatus = response.data.followers != null ? response.data.followers.find(obj => obj.userid === localStorage.getItem('token')) : false
